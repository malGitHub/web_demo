(function () {
    ...
    angular
        .module('WeViews')
        .controller('TserviceRoleRoleeditController', TserviceRoleRoleEditController);
    /**
     *
     ...
     * @param Message
     */
    function TserviceRoleRoleEditController($rootScope,$uibModalInstance, RuleService, Message, name, description, id, suportIds, systemIds, sourceAuthIds, marketIds, subscribeIds) {
        var vm = this;
        vm.allCheckArray=[];
        vm.buessinesList=[];
        vm.systemList=[];
      ...
      vm.checkIt=function (object) {
        ...
        //修改内容
        vm.applyUserPhone = name;
        vm.backUp = description;
        ...
        function getBuessinesList() {
            RuleService.qyeryFuncList("1")
                .then(function (data) {
                    ...
                    //初始化设置以及被选中的业务支撑
                    if (suportIds != null && suportIds != '' && suportIds != undefined) {
                        vm.checkRequest=true;
                        vm.buessinesList.forEach(function (item) {
                            if (suportIds.indexOf(item.id) != -1) {
                                item.checked = true;
                            } else {
                                item.checked = false;
                            }
        ...
        function getsystemListList() {
            RuleService.qyeryFuncList("2")
                .then(function (data) {
                    ...
                    //初始化设置以及被选中的系统设置
                    if (systemIds != null && systemIds != '' && systemIds != undefined) {
                        vm.checkRequest=true;
                        vm.systemList.forEach(function (item) {
                            if (systemIds.indexOf(item.id) != -1) {
                                item.checked = true;
                            } else {
                                item.checked = false;
                            }
        ...
        function getSourceAuthList() {
          RuleService.qyeryFuncList("3")
            .then(function (data) {
              ...
              //初始化设置以及被选中的货源会员认证
              if (sourceAuthIds != null && sourceAuthIds != '' && sourceAuthIds != undefined) {
                vm.checkRequest=true;
                vm.sourceAuthList.forEach(function (item) {
                  if (sourceAuthIds.indexOf(item.id) != -1) {
                    item.checked = true;
                  } else {
                    item.checked = false;
                  }
        ...
        function getMarketList() {
          RuleService.qyeryFuncList("16")
            .then(function (data) {
              ...
              //初始化设置以及被选中的市场推广
              if (marketIds != null && marketIds != '' && marketIds != undefined) {
                vm.checkRequest=true;
                vm.marketList.forEach(function (item) {
                  if (marketIds.indexOf(item.id) != -1) {
                    item.checked = true;
                  } else {
                    item.checked = false;
                  }
      ...
      function getSubscribeList() {
        RuleService.qyeryFuncList("19")
          .then(function (data) {
            ...
            //初始化设置以及被选中的服务预约管理
            if (subscribeIds != null && subscribeIds != '' && subscribeIds != undefined) {
              vm.checkRequest=true;
              vm.subscribeList.forEach(function (item) {
                if (subscribeIds.indexOf(item.id) != -1) {
                  item.checked = true;
                } else {
                  item.checked = false;
                }
      ...
      vm.forAllChectFunc=function () {
        ...
        vm.submit = function (evt, form) {
            ...
            if (form.$valid && !vm.requesting) {
                vm.requesting = true;
                RuleService.editRole(vm.applyUserPhone, vm.backUp, getFuncIds(), id,vm.userId)
                    .then(function () {
