(function () {
  ...
  angular.module('WeViews')
    .controller('TserviceActivitiesnewtwoImportController', TserviceActivitiesnewtwoImportController);

  function TserviceActivitiesnewtwoImportController($rootScope,$uibModalInstance,TbossService, ActivityListService,id,carChassisNumList, Message,$timeout) {
    var vm = this;
    vm.file = null;
    vm.cancel = function () {
      ...
      if(vm.file.name.match(/\.(\w+)$/)[1]!="xlsx" && vm.file.name.match(/\.(\w+)$/)[1]!="xls"){
        ...
        function fileUpload(data) {
          ...
          ActivityListService.carChassisNumUpload(data.data.fullPath,data.data.ext_name,id,strings)
            .then(function (data) {
              /*data.carChassisNumList = carChassisNumList.concat( data.carChassisNumList);*/
